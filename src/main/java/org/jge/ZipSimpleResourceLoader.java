...
public class ZipSimpleResourceLoader extends ResourceLoader
{
	...
	private ResourceLocation zipFile;
    
    public ZipSimpleResourceLoader(DiskSimpleResource res)
    {
    	this.zipFile = res.getResourceLocation();
    }
    ...
    private InputStream getInputStream(ResourceLocation location)
	{
    	try
        {
            String absolutePath = zipFile.getFullPath();
            ZipInputStream inputStream = new ZipInputStream(new FileInputStream(absolutePath));
            ZipEntry e;
